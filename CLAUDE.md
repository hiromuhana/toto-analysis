...
バックテスト実績（適正バリデーション済み）:
- DCモデル的中率: 49.7%（ランダム33.3%の1.5倍）
- CatBoost(DC+Pi): 50.3%（微改善、差は僅少）
- ダブル的中率: 74.9%
- minitoto推奨（ROI +751%）
...
1. **統計モデルは統計モデルとして保つ** — DC素 or CatBoost基本10特徴量を使う
2. **LLMは「データに入ってない事実」の補正に限定** — 怪我、監督交代、降格文脈等
3. **LLMの提案で特徴量を追加する場合** — Train(50%)/Valid(25%)/Test(25%)の3分割を厳守
4. **Valid後半で改善した特徴量のみ採用** — 悪化は棄却
5. **Test(封印)セットは最終評価まで絶対に見ない**

根拠: 2026-03-28のバックテストで「LLM提案の新特徴量で引分け17.3%に改善」と見えたが、
適正バリデーションでは3.1%に悪化。テストデータの失敗分析→同じデータで評価 = 過剰適合の典型例。
